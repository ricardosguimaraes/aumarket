<div id="ws-ame-menu-heading-settings" title="Menu Headings" style="display: none;">
	<div class="ws_dialog_panel">
		<div class="ame-scrollable-dialog-content">
			...
			function amePrintHeadingSpacingTypeOption($spacing, $value, $label) {
				echo '<p><label>';
				printf(
					'<input type="radio" name="ame-heading-%1$s-type"
						    value="%2$s" data-bind="checked: settings.%1$sType">',
					esc_attr($spacing),
					esc_attr($value)
				);
				echo ' ', esc_html($label);
			...
			function amePrintSpacingSizeInput($spacing, $side, $label) {
				printf(
					'<label>
						<span class="ame-fixed-label-text">%s:</span>
						<input type="number" min="0" max="300" data-bind="value: settings.%s%s"> px
					</label>',
					esc_html($label),
					esc_attr($spacing),
					esc_attr(ucfirst($side))
				);
			}

			foreach ($spacingTypes as $spacing => $spacingLabel):
				?>
				<div class="ws_dialog_subpanel">
					<h3 class="ws-ame-dialog-subheading"><?php echo esc_html($spacingLabel); ?></h3>
					<fieldset>
						<?php amePrintHeadingSpacingTypeOption($spacing, 'auto', 'Automatic'); ?>
						<?php amePrintHeadingSpacingTypeOption($spacing, 'custom', 'Custom'); ?>
					</fieldset>
					<fieldset class="ame-box-side-sizes"
					          data-bind="<?php echo esc_attr(sprintf(
						          'enable: (settings.%sType() === \'custom\')',
						          $spacing
					          )) ?>">
						<?php amePrintSpacingSizeInput($spacing, 'top', 'Top'); ?>
						<?php amePrintSpacingSizeInput($spacing, 'bottom', 'Bottom'); ?>
						<div class="ame-flexbox-break"></div>
						<?php amePrintSpacingSizeInput($spacing, 'left', 'Left'); ?>
						<?php amePrintSpacingSizeInput($spacing, 'right', 'Right'); ?>
					</fieldset>
				</div>
			<?php endforeach; ?>
